...
\begin{equation*}
\boxed{
    \frac{\partial c}{\partial t} = \frac{D}{\textcolor{red}{R(c)}} \frac{\partial^2 c}{\partial x^2}
}
\end{equation*}
}
}
...
\centering
\only<2>{\hspace{0.8cm} \textbf{Train} \hspace{2.0cm} \textbf{Test} \hspace{2.2cm} \textbf{Test}}
...
\begin{tabular}{rlrl} 
    ...
    $p(\theta)$: & Prior & \only<3->{$\hat{R}(c;\theta)$:} & \only<3->{Predicted Retardation Factor} \\
\end{tabular}
